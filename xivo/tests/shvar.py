...
class TestShVar(unittest.TestCase):
    ...
    def test_load(self):
        ...
        self.assertEqual(
            shvar.load([" \t\n", "# comment\n", "a=1"]),
            ([(None, None, ""), (None, None, "# comment"), ('a', "1", "")], {'a': "1"}),
        )
        self.assertEqual(
            shvar.load(["a=1\n", "a=2\n"]),
            ([('a', "1", ""), ('a', "2", "")], {'a': "2"}),
    ...
    def test_load_unsup(self):
        self.assertRaises(
            shvar.UnsupportedAssignmentError, shvar.load, ["a=`echo lol`\n"]
        )
        self.assertRaises(
            shvar.UnsupportedAssignmentError, shvar.load, ["a=$(echo lol)\n"]
        )
        self.assertRaises(
            shvar.UnsupportedAssignmentError, shvar.load, ["a=~homesweethome\n"]
        )
        self.assertRaises(
            shvar.UnsupportedAssignmentError, shvar.load, ["a=abc:~homesweethome\n"]
        )
        ...
        self.assertRaises(
            shvar.UnsupportedAssignmentError, shvar.load, ["a=$((1+1))\n"]
        )
        self.assertRaises(
            shvar.UnsupportedAssignmentError, shvar.load, ['a=$"to be translated"\n']
        )
    ...
    def test_load_compositing(self):
        self.load_value_helper(
            "abc\\ \"bla bla\"'\tkikoo'$'\\x26'", "abc bla bla\tkikoo&"
        )
    ...
    def test_strip_overridden_assignments(self):
        ...
        self.assertEqual(
            shvar.strip_overridden_assignments([("abc", "def", "")]),
            [("abc", "def", "")],
        )

        param = [("abc", "def1", ""), ("ghi", "def2", "")]
        expect = param[:]
        self.assertEqual(shvar.strip_overridden_assignments(param), expect)

        param = [(None, None, "# lol"), ("abc", "def1", ""), ("ghi", "def2", "")]
        expect = param[:]
        self.assertEqual(shvar.strip_overridden_assignments(param), expect)
        ...
        param = [("abc", "def_rm", ""), ("abc", "def", "")]
        expect = param[-1:]
        self.assertEqual(shvar.strip_overridden_assignments(param), expect)

        param = [(None, None, ""), ("abc", "def_rm", ""), ("abc", "def", "")]
        expect = param[::2]
        self.assertEqual(shvar.strip_overridden_assignments(param), expect)

        param = [("abc", "def_rm", ""), ("abc", "def", ""), (None, None, "")]
        expect = param[-2:]
        self.assertEqual(shvar.strip_overridden_assignments(param), expect)
    ...
    def format_helper(self, initial_content):
        ...
        # 4) check
        self.assertEqual(
            [(a, b) for a, b, c in reslst if a], [(a, b) for a, b, c in new_reslst if a]
        )

    def test_format(self):
        self.format_helper(["A=\n"])
        self.format_helper(["A=1\n", "B=a\n"])
        self.format_helper(["# comment 1\n", "A=1 # comment 2\n", "B=a#b\n"])
        self.format_helper(['A="\xe9"\n'])
